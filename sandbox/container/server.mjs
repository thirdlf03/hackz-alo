  ...
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/read') {
      ...
      const data = await readFile(
        path,
        body.encoding === 'base64' ? undefined : 'utf8'
      );
      return json(response, {
        success: true,
        path: body.path,
    ...
    if (request.method === 'POST' && url.pathname === '/api/exists') {
      ...
      return json(response, {
        success: true,
        path: body.path,
        exists: existsSync(safePath(body.path)),
        timestamp: timestamp(),
      });
    }
    ...
    if (
      request.method === 'POST' &&
      (url.pathname === '/api/rename' || url.pathname === '/api/move')
    ) {
      const body = await readJson(request);
      ...
      return json(response, {
        success: true,
        path: body.newPath ?? body.destinationPath,
        timestamp: timestamp(),
      });
    }
    ...
    if (processMatch) {
      ...
      if (request.method === 'GET' && action === 'stream') {
        return sse(response, [
          {type: 'stdout', text: proc.stdout},
          {type: 'stderr', text: proc.stderr},
          {type: 'complete'},
        ]);
      }
...
function watchPort(response, body) {
  ...
  const interval = setInterval(
    async () => {
      ...
      if (ready) {
        response.write(
          `data: ${JSON.stringify({type: 'ready', port: Number(body.port), timestamp: timestamp()})}\n\n`
        );
        clearInterval(interval);
        response.end();
      } else if (Date.now() > timeoutAt) {
        response.write(
          `data: ${JSON.stringify({type: 'timeout', port: Number(body.port), timestamp: timestamp()})}\n\n`
        );
        clearInterval(interval);
        response.end();
      } else {
        response.write(
          `data: ${JSON.stringify({type: 'checking', port: Number(body.port), timestamp: timestamp()})}\n\n`
        );
      }
    },
