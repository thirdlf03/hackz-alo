...
export const ASSIST_SYSTEM_PROMPT = [
  ...
  '次の一手のコマンドは、画像に写っている文字列をそのままコピーしてください。画像にないコマンド名を作らないでください(一般知識のコマンド名の捏造は禁止です)。Runbookの注意書きや方針・精神論(例:「再起動しても再発する」)を次の一手にしないでください。画像と質問文から答えの根拠が見つからない場合は、コマンドを作らずに次の一手へ「不足:」に続けて知りたい情報を書いてください。',
