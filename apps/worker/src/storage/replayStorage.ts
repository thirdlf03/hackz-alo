...
export async function putReplayChunk(
  ...
  await env.DB.prepare(
    ...
    .run();
  await env.REPLAY_BUCKET.delete(replayVideoKey(input.replayId)).catch(
    () => undefined
  );
  await env.DB.prepare(
    `update replays
...
export async function putReplayEvents(
  env: Bindings,
  replayId: string,
  seq: number,
  events: ReplayEvent[]
  ...
  const manifest = await putReplayEventsManifest(
    env,
    replayId,
    await listReplayEventKeys(env, replayId)
  );
  await env.DB.prepare(
    'update replays set event_log_object_key = ?, updated_at = ? where id = ?'
  )
...
export async function uploadMultipartPart(
  ...
  const upload = env.REPLAY_BUCKET.resumeMultipartUpload(
    row.object_key,
    row.upload_id
  );
  const uploadedPart = await upload.uploadPart(input.partNumber, input.body);
  const parts = JSON.parse(row.uploaded_parts_json) as Array<{
    partNumber: number;
    etag: string;
  }>;
  const nextParts = [
...
export async function completeMultipartUpload(env: Bindings, replayId: string) {
  ...
  await env.DB.batch([
    env.DB.prepare(
      'update replay_multipart_uploads set status = ?, updated_at = ? where replay_id = ?'
