...
test('unlang CLI masks structured runtime errors from player-facing output', async () => {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [unlangCli, 'run', file], {
      encoding: 'utf8',
    });
    ...
    assert.doesNotMatch(
      result.stderr,
      /DIVISION_BY_ZERO|line|column|100|うんわり/u
    );
  } finally {
...
test('unlang CLI check prints ok for valid source without the computed result', async () => {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [unlangCli, 'check', file], {
      encoding: 'utf8',
    });
