...
test("all scenario data validates against shared contract", async () => {
  ...
  for (const { file, scenario } of scenarios) {
    ...
    assert.deepEqual(result.ok ? [] : result.errors, [], `${file} should be valid`);
...
test("scenario validator rejects incomplete success conditions", () => {
  ...
  assert.match(errors, /successConditions\[0\]\.status must be a finite number/);
});
...
test("scenario validator rejects malformed trigger params", () => {
  ...
  assert.match(errors, /triggers\[0\]\.params\.bytes must be a positive integer/);
});
...
test("scenario validator rejects unsupported alert enums", () => {
  ...
  const errors = expectInvalidScenario(scenario);
  assert.match(errors, /alerts\[0\]\.severity must be info, warning, or critical/);
  assert.match(errors, /alerts\[0\]\.source must be scenario or monitor/);
});

async function loadScenarios() {
  const files = (await readdir(scenarioDataDir)).filter((file) => file.endsWith(".json")).sort();
  return Promise.all(
    files.map(async (file) => ({
      file,
      scenario: JSON.parse(await readFile(path.join(scenarioDataDir, file), "utf8"))
    }))
  );
}
